    ...
    for (int v : e[u])
        if (v != fr) {
            ...
            if (cnt[v].size() > cnt[u].size())
                swap(cnt[u], cnt[v]), swap(sum[u], sum[v]);
            for (auto p : cnt[v])
                sum[u][cnt[u][p.first]] -= p.first, cnt[u][p.first] += p.second, sum[u][cnt[u][p.first]] += p.first;
        }
    ans[u] = (*(--sum[u].end())).second;
}
void solve(int cas) {
    cin >> n;
